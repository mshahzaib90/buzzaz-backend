...
const ChatWindow = ({ conversationId, currentUser, onClose }) => {
  ...
  // Conversation polling effect is defined after fetchMessages to avoid
  // no-use-before-define warnings in strict CI builds
  ...
  // Start polling once conversationId is set and the fetch function is defined
  useEffect(() => {
    if (conversationId) {
      fetchMessages();
  ...
  return (
    <Card className="h-100 d-flex flex-column chat-window-card">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="chat-avatar position-relative">
            <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden' }}>
              <img
                src={otherParticipant?.avatarUrl || (otherParticipant?.avatar ? getUploadsUrl(otherParticipant.avatar) : `https://i.pravatar.cc/80?u=${otherParticipant?.id || ''}`)}
                alt="avatar"
                style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                onError={(e) => { e.currentTarget.src = `https://i.pravatar.cc/80?u=${otherParticipant?.id || ''}`; }}
              />
            </div>
          </div>
          <div>
      ...
      {/* Messages Area */}
      <Card.Body className="flex-grow-1 p-0 chat-gradient-bg" style={{ overflowY: 'auto' }}>
        {error && (
