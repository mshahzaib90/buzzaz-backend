...
const ChatList = ({ currentUser, onSelectConversation, selectedConversationId }) => {
  ...
  const filteredUsers = availableUsers
    .filter(user => modalRoleFilter === 'all' || user.role === modalRoleFilter)
    .filter(user =>
                        ...
                        <div className="d-flex align-items-start gap-3">
                          <div className="chat-avatar position-relative">
                            <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden' }}>
                              <img
                                src={otherParticipant?.avatarUrl || (otherParticipant?.avatar ? getUploadsUrl(otherParticipant.avatar) : `https://i.pravatar.cc/80?u=${otherParticipantId}`)}
                                alt="avatar"
                                style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                                onError={(e) => { e.currentTarget.src = `https://i.pravatar.cc/80?u=${otherParticipantId}`; }}
                              />
                            </div>
                          </div>
                    ...
                    {filteredUsers.map((user) => (
                      ...
                      >
                        <div className="d-flex align-items-center gap-3">
                          <div style={{ width: 36, height: 36, borderRadius: '50%', overflow: 'hidden', background: '#e9ecef' }}>
                            <img
                        ...
                        <Badge bg={
                          user.role === 'influencer' ? 'warning' :
                          user.role === 'brand' ? 'primary' :
                          user.role === 'ugc_creator' ? 'success' :
                          'secondary'
                        }>
                          {user.role === 'ugc_creator' ? 'ugc' : String(user.role || 'user')}
                        </Badge>
                      </ListGroup.Item>
                    ))}
                  </ListGroup>
