...
// Rate limiters (balanced for dev; stricter in prod)
const isProd = String(process.env.NODE_ENV).toLowerCase() === 'production';
const loginLimiter = rateLimit({
  windowMs: isProd ? 60 * 1000 : 30 * 1000,
  max: isProd ? 5 : 20,
...
router.post('/register', registerLimiter, [
  ...
  body('role').isIn(['brand', 'content_creator', 'influencer', 'ugc_creator', 'admin', 'support', 'seeder']).withMessage('Invalid role selected')
], async (req, res) => {
  try {
